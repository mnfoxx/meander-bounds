...
// Usage: relax_big k iters total LOG2HASH STATECAP   e.g.  relax_big 40 600 28 30 350000000
...
uint64_t*eoff;uint32_t*edst;size_t nedges=0,ecap;
int main(int argc,char**argv){
  ...
  if(pass==1){eoff=malloc((nst+1)*8);ecap=nst*4;edst=malloc(ecap*4);nedges=0;fprintf(stderr,"states %zu; allocating %zu edge slots\n",nst,ecap);}
  for(size_t i=0;i<nst;i++){
    ...
    for(int er=0;er<2;er++)for(int el=0;el<2;el++){
      ...
      if(pass==1)edst[nedges++]=(uint32_t)j;}}
  }
  ...
  for(int it=0;it<iters;it++){ // right vector: v <- T^2 v  (v_i = sum_j T_ij v_j)
    for(int h=0;h<2;h++){
      ...
      for(size_t i=0;i<nst;i++){double s=0;for(uint64_t k=eoff[i];k<eoff[i+1];k++)s+=v[edst[k]];u[i]=s;}
      double mx=0;for(size_t i=0;i<nst;i++)if(u[i]>mx)mx=u[i];
      #pragma omp parallel for schedule(static)
      for(size_t i=0;i<nst;i++)v[i]=u[i]/mx;if(h==0)lam=mx;else lam*=mx;}
  ...
  t2=(int64_t*)v;
  #pragma omp parallel for schedule(static)
  for(size_t i=0;i<nst;i++){int64_t s=0;for(uint64_t k=eoff[i];k<eoff[i+1];k++)s+=w[edst[k]];t1[i]=s;}
  #pragma omp parallel for schedule(static)
  for(size_t i=0;i<nst;i++){int64_t s=0;for(uint64_t k=eoff[i];k<eoff[i+1];k++)s+=t1[edst[k]];t2[i]=s;}
