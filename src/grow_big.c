...
// Large-machine build: table sizes from the environment, GROW_LOG2HASH (default 27; use 30 for 4e8 states) and
// GROW_LOG2FRONT (default 25; use 28 on a 128 GB machine). Compile: gcc -O2 -fopenmp -o grow_big grow_big.c -lm
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
int main(int argc,char**argv){
  ...
  cap=1<<22;states=malloc(cap*sizeof(St));{const char*e=getenv("GROW_LOG2HASH");hcap=1ULL<<(e?atoi(e):27);}htab=malloc(hcap*4);hash_clear();
  {const char*e=getenv("GROW_LOG2FRONT");fcap=1ULL<<(e?atoi(e):25);}ftab=malloc(fcap*sizeof(FE));
  fprintf(stderr,"hash slots %zu, frontier slots %zu\n",hcap,fcap);
